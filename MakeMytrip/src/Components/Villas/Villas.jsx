import * as React from 'react';
import Box from '@mui/material/Box';
import Tab from '@mui/material/Tab';
import TabContext from '@mui/lab/TabContext';
import TabList from '@mui/lab/TabList';
import TabPanel from '@mui/lab/TabPanel';
import './Villas.css'
function Villas() {
  const [value, setValue] = React.useState('1');

  const handleChange = (event, newValue) => {
    setValue(newValue);
  };

  return (
    <Box sx={{ width: '100%', typography: 'body1' }}>
      <TabContext value={value}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <TabList onChange={handleChange} aria-label="lab API tabs example">
            <Tab label="Hotels" value="1" />
            <Tab label="All Offers" value="2" />
            <Tab label="Flights" value="3" />
            <Tab label="Holidays" value="4" />
            <Tab label="Trains" value="5" />
            <Tab label="Cabs" value="6" />            
            <Tab label="Bank Offers" value="7" />           
            <Tab label="Activities" value="8" /> 
            
          </TabList>
        </Box>
        <TabPanel value="1">Hotels</TabPanel>
        <TabPanel value="2">All Offers</TabPanel>
        <TabPanel value="3">Flights</TabPanel>
        <TabPanel value="4">Holidays</TabPanel>
        <TabPanel value="5">Trains</TabPanel>
        <TabPanel value="6">Cabs</TabPanel>
        <TabPanel value="7">Bank Offers</TabPanel>
        <TabPanel value="8">Activities</TabPanel>
      </TabContext>
    </Box>
  );
}
export default Villas